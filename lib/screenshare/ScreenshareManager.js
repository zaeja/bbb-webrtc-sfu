...
module.exports = class ScreenshareManager extends BaseManager {
  constructor (connectionChannel, additionalChannels, logPrefix) {
    ...
    // listen for presenter change and user disconnection to avoid inconsistent states on reconnection
    this._trackPresenterChangeEvent();
    this._trackUserLeftMeetingEvent();
  }
  ...
  _trackUserLeftMeetingEvent () {
    this._bbbGW.on(C.USER_LEFT_MEETING_2x, (payload) => {
      const meetingId = payload[C.MEETING_ID_2x];
      const userId = payload[C.USER_ID_2x];
      const sessionId = this._meetings[meetingId];
      const session = this._fetchSession(sessionId);
      if (session) {
        const screenshareMeta = session._getFullPresenterLogMetadata();
        if (screenshareMeta.userId === userId) {
          Logger.info(this._logPrefix, `User left meeting, forcibly closing screensharing session`,
              screenshareMeta);
          const queue = this._fetchLifecycleQueue(sessionId);
          queue.push(() => {
            return this.closeSession(session, C.SEND_ROLE, sessionId);
